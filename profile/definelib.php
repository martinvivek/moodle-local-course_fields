...
function profile_delete_category($id) {
	...
	*/
    ...
    if ($DB->count_records('course_info_field', array('categoryid'=>$category->id))) {
...
function profile_move_field($id, $move) {
    ...
    if ($swapfield = $DB->get_record('course_info_field', array('categoryid'=>$field->categoryid, 'sortorder'=>$neworder), 'id, sortorder')) {
	...
	*/
    ...
    if ($fieldform->is_cancelled()) {
        ...
        if ($data = $fieldform->get_data()) {
            require_once($CFG->dirroot.'/local/course_fields/profile/field/'.$datatype.'/define.class.php');
